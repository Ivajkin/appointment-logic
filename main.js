...
$(document).ready(function() {
	...
	(function() {
		...
		var test_result_dates_2 = [{date: "01.01.2016", time:["12:50","13:50"]}];
	...
	(function() {
		var date = new Date();
		var test_date_1 = date.getDate() + "." + date.getMonth() + ".2015";
		var test_date_2 = date.getDate()+4 + "." + (date.getMonth()+1) + ".2015";
		...
		*/
		...
		function open_print_page() {
			...
			function Popup(data) {
				...
				mywindow.document.write('<html><head><title>ЦПСБИЗ</title>');
		...
		for(var key in specialists) {
			if(specialists[key].talons)
				schedule += '<tr> <td> <a class="doctor-selector" href="javascript:void(0);">'+specialists[key].name+'</a></td><td>'+
					specialists[key].speciality + 	'</td></tr><!--<tr>-->';
		}
		...
		$('.doctor-selector').click(function() {
			...
			var schedule = '<form id="appointment" method="post" action="">' +
			    		'<p>Врач: ' + doctor + '</p>' +
				    	'<p><span id="patient_label" class="text_marked">Введите Вашу фамилию, инициалы в графу Пациент</span><br>Пациент: <input type="text" style="width: 300px; margin-top: 5px" name="patient_name" id="patient_name" value=""></p>' +
				    	'<p style="margin-bottom: 15px; margin-top: 20px"><span id="date_label" class="text_marked">Выберите дату и время записи.</span></p>' +
				    	'<div class="time-selector" style="margin: -15px 10px 20px 150px;" id="div_28.09.2015">' +

				    	'<p><input type="hidden" name="send_app"></p><p><input type="hidden" name="doctor_id" id="doctor_id">'+doctor_id+'</p></form>';
			...
			//	function() {
						...
						for(var key in talons_by_doctors[doctor]) {
							...
							for(var key2 in talons_by_doctors[doctor][key].time) {
								...
								talons += '<input class="time-selector-time" type="radio" id="Дата: ' + date + '. Время: ' + time + '" name="time" value="' + date + ' ' + time + '">' + time + '<br>';
							}
						}
						//talons_by_doctors[doctor][key].date + talons_by_doctors[doctor][key].time;

						//$('.time-selector').html( +
						//		'<input class="time-selector-time" type="radio" name="time" id="Дата: 28.09.2015 Время: 11:40" value="1443400800" onclick="radio_changed()">  11:40<br></div>');
